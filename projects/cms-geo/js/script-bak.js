...
// column (bar) chart of states by % of population w/ diabetes
// better would be to show +/- national average, animated by year

// data across all years
var data; // loaded asynchronously using d3.json
var yearData;
...
var indicatorBarChart,
    ...
    displayYear, // the year currently being displayed
    minYear,
    maxYear,
    minVal,
    maxVal,
...
var stateMapData = "./data/us-states.json";
...
var getYearData = function(theData, theYear) {
    return _.select(theData, function(element, index) { return element[yr] === theYear; });
};
...
function loadData(file) {
    d3.json(file, function(json) {
        ...
        minVal = d3.min(data, function(d) { return d[val]; });

        minYear = d3.min( data, function(d) { return d[yr]; } ),
        maxYear = d3.max( data, function(d) { return d[yr]; } );

        // set default display year to most recent
        displayYear = maxYear;
        ...
        for ( var i = 0 ; i < uniqYears.length ; i++ ) {
           var y = uniqYears[i];
           var dataYears = getYearData(data, y);
...
function initYearData() {
    ...
    // data for display year only
    yearData = getYearData( data, displayYear );

    // sort by value by default
    yearData.sort(function(a,b) { return b[val] - a[val]; });
    alphaSort = false;
    ...
    mean = _.select(avgYears, function(element, index) {
                return element[yr] === displayYear; })[0][avg];
...
function initIndicatorBars() {
    ...
    // TODO: may be an issue if length changes each year
...
function initMap() {
    ...
    stateMap = map.append("svg:g");

    d3.json(stateMapData, function(json) {
...
function colorMapLocale(data) {
    var id = +data['id'];
    var el = _.select(yearData, function(element, index) { return element["Locale Level"] === "State" && element["Locale State FIPS Code"] === id; })[0];
    if ( _.isUndefined(el) )
        return "#888";
    else
...
function initYearHistogram() {
    ...
    .enter().append("svg:g")
        ...
        .on("click", function(d) {
            displayYear = d[yr];
            initYearData();
            yearHistogramTitle.text(displayYear); // update title for selected year
...
function initBarRects() {
    ...
    .enter().append("svg:g")
        ...
        .on("mouseover", function (d, i) {
            highlightBarRects(d["Locale State FIPS Code"], true);
            d3.select("#map-" + d["Locale State FIPS Code"])
                .style("fill", highlightColor);
        })
        .on("mouseout", function (d, i) {
            highlightBarRects(d["Locale State FIPS Code"], false);
            d3.select("#map-" + d["Locale State FIPS Code"])
                .style("fill", function(d) {return colorMapLocale(d);});
        });
    ...
    indicatorRects = indicatorBars.append("svg:rect")
        .attr("id", function(d) { return "bar-" + d["Locale State FIPS Code"]; })
    ...
    indicatorBars.append("svg:text")
        .attr("id", function(d) { return "bar-label-" + d["Locale State FIPS Code"]; })
        .attr("class", "label")
        .attr("x", -8)
    ...
    indicatorBars.append("svg:text")
        .attr("id", function(d) { return "bar-value-" + d["Locale State FIPS Code"]; })
        .attr("class", "barLabel")
        .attr("display", "none")
...
function transitionSortNumeric() {
    ...
    d3.select("#indicatorBarChart").selectAll("g.bar")
        .data(yearData.sort(function(a,b) { return b[val] - a[val]; }),
...
function transitionSortAlphabetic() {
    ...
    d3.select("#indicatorBarChart").selectAll("g.bar")
        .data(yearData.sort(function(a, b) { return d3.ascending(a[name], b[name]); }),
            function(d) { return d[name]; })
    .transition()
        .delay(function(d, i) { return i * 10; })
        .attr("transform", function(d,i) { return "translate(0," + yIndicatorScale(i) + ")"; });
